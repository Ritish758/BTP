...
int bestmatch(vector<int>& time, int cur_time, Task t){
  ...
  // cout<<time[1]<<" " <<cur_time<<"\n";
   
  // cout<<"Task "<<t.tid<<" "<<bestproc<<"\n"; 
...
int bestprocdrop(vector<vector<Task>>& proc, Task t, vector<int>& time, int cur_time){
 ...
 if(bestproc!=-1){
    cout<<time[bestproc]<<"\n";
  for(auto x: proc[bestproc]){
    cout<<x.tid<<" "<<x.flag<<" "<<x.process_time<<"\n";
...
void schedule_greedy1(vector<Task>& tasks, int m){
   ...
   for(int j=1; j<=m; j++){
      ...
      if(x.start_time==-1){
        ...
        if(x.end_time <=x.deadline){cout<<"ok\n";}
        else{cout<<"notok\n";}
     ...
     for(auto x: intask){
      ...
      if(proc!=-1){
       ...
       if(x.flag==0){
          ...
          for(int j=0; j<processors[proc].size(); j++){
             if(k<droped.size() && j==droped[k]){
             if(processors[proc][j].start_time!=-1){time[proc]-= (processors[proc][j].end_time - t-1); cout<<"aborted task "<<"\n";}
             else{time[proc]-= (processors[proc][j].process_time);}
             cout<<"Task "<<processors[proc][j].tid<<" Drooped soft-hard\n";
             dropsoft++;
             k++;
             continue;
             }
