...
int main(){
    ...
    vector<int> task_count = {100};  // total different tasks
    vector<int> processor = {500}; // total different processors
    ...
    for(auto tskc : task_count){
        for(auto proc : processor){
            ...
            set<pair<int,int>> s;  // store the base stations and vehicular unit's as they should be different
            int temp = proc;
            while(temp--){  // print random locations of bs
              int x1 = randI(1,grid);
              int y1 = randI(1,grid);
              
              if(s.find({x1,y1})!=s.end()){temp++; continue;}
              s.insert({x1,y1});
            ...
            for(int i=0; i<tskc; i++){
                ...
                if(process_time==0 || deadline-ar_batch <=1 || s.find({x,y}) != s.end() ){i--; continue;}
