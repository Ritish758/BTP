...
int mant_calc_dis(int x, int y, int id){
    return abs(x-base_station[id].first)+ abs(y-base_station[id].second);
}

bool d_hop(int x, int y, int id, int d){
     d = min(dis_hop,d);
     ...
     int val = check[d-1].first;
     if(abs(x-base_station[id].first)*abs(x-base_station[id].first) + abs(y-base_station[id].second)*abs(y-base_station[id].second) <= val){return true;}
     return false;
